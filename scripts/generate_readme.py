#!/usr/bin/env python3
"""
generate_readme.py — Repo MD
Scanne writeups/*.md, lit les métadonnées, et met à jour uniquement
les sections entre <!-- STATS:START/END --> et <!-- WRITEUPS:START/END -->
dans README.md. Le reste du README (logo, présentation...) est préservé.
"""
import re
from pathlib import Path
from datetime import datetime

README_TITLE       = "LAB-OSINT-Writeups"
README_DESCRIPTION = "Résumés (writeups) pour les challenges du site LAB'OSINT."
PLATFORM_LABEL     = "LAB'OSINT"
BADGE_COLOR        = "2ea043"
STATS_TITLE        = "📊 LAB'OSINT Stats"
...
def extract_meta_html(filepath):
    text = filepath.read_text(encoding="utf-8")
...
def main():
    ...
    if WRITEUPS_DIR.exists():
        pattern = f"*.{FORMAT}"
        for f in sorted(WRITEUPS_DIR.glob(pattern)):
            if f.stem.upper() == "TEMPLATE":
                continue
            meta = extract_meta_md(f) if FORMAT == "md" else extract_meta_html(f)
            if meta:
                entries.append(meta)
